...
DimPlot(hNeut_GSE127465_s, group.by = "seurat_clusters",label = T)
...
plotGeneCount(curves, filt_counts, clusters = hNeut_GSE127465_s.sce$seurat_clusters, models = sce)
...
#cxcl2
Start <- order(startRes$waldStat, decreasing = TRUE)
sigGeneStart <- names(sce)[oStart[1185]]
plotSmoothers(sce, counts, gene = sigGeneStart)
