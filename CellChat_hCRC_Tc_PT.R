...
LR.show <- pairLR.IFN[1,] # show one ligand-receptor pair

# show all the significant interactions (L-R pairs) from some cell groups (defined by 'sources.use') to other cell groups (defined by 'targets.use')
netVisual_bubble(cellchat, sources.use = 1, targets.use = c(2), remove.isolate = FALSE)
netVisual_bubble(cellchat, sources.use = 2, targets.use = c(1), remove.isolate = FALSE)


plotGeneExpression(cellchat, signaling = "MIF")
plotGeneExpression(cellchat, signaling = "CLEC", enriched.only = FALSE)
...
gg2 <- netAnalysis_signalingRole_scatter(cellchat, signaling = c("IFN-II"))
gg2
